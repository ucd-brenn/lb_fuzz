...
i = 6
county_code, county_letters = counties[i]
print(county_letters)

o_df = pd.read_csv('OutFilesIndiv/DE.csv')
o_uids = o_df.uid.unique().tolist()
...
p_df.query('uid not in @o_uids', inplace = True)
len(p_df)
...
p_df.uid.nunique() - 234931
p_df.site_street_name.value_counts()

p_df['test_add']  = p_df.apply(lambda r: ' '.join([c for c in [str(r.site_direction), str(r.site_street_name), str(r.site_mode), str(r.site_unit_prefix), str(r.site_unit_number)] if c != 'None' and c != 'nan']), axis = 1)
# p_df.loc[171898].site_house_number
p_df[p_add_cols].sample(5)
p_df.site_street_name.value_counts()
p_df.query('site_street_name == "TEMPLE"')[p_add_cols]
len(p_df)
...
c_df[c_df.situs.str.contains('N CHEROKEE ST APT 311')==True]
c_df[(c_df.situs.str.contains('1325')==True) & c_df.situs.str.contains('ACOMA  ')==True]


c_df.match_id = c_df.match_id.str.upper()
j_options = c_df.match_id.unique().tolist()

process.extract('CHEROKEE ST APT 311', j_options)
